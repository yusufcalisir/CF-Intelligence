...
| Component / Function | Formal Specification | Security / Data Claim | Verification Status | Scientific Classification |
|:---|:---|:---|:---:|:---:|
| **HMAC-SHA256 Identity Anonymization** | $h_i = \text{HMAC-SHA256}(\text{salt}, \text{raw\_id}_i)$ | Non-reversible, deterministic PII hashing | 4/4 Pass ($|h_i| = 64$ hex) | 🟢 **SUPPORTED** |
| **Dirichlet Non-IID Data Partitioning** | $(p_1, \dots, p_K) \sim \text{Dir}(\alpha \cdot \mathbf{1}_K)$ | Simulates statistical heterogeneity across banks | 4/4 Pass ($\sum N_k = N_{\text{total}}$) | 🟢 **SUPPORTED** |
...
| **PyArrow Zero-Copy Parquet Export** | Snappy compressed columnar layout | High-throughput binary streaming without serialization overhead | 4/4 Pass | 🟢 **SUPPORTED** |
| **Dataloader Auto-Discovery** | glob `storage/datasets/<name>/*.parquet` | Seamless benchmark & FL engine integration | Verified via `load_dataset("paysim")` | 🟢 **SUPPORTED** |
| **Label Ratio Non-Uniformity** | $\text{Var}\left(\frac{\text{Fraud}_k}{N_k}\right) > 0$ for $\alpha < \infty$ | Realistic Non-IID fraud imbalance across banks | Verified across 3 bank partitions | 🟢 **SUPPORTED** |
| **Synthetic Mock Fallback** | Fallback to `n_mock_txns` when raw CSV missing | Robust pipeline operation without external dependencies | Verified in CI environment | 🟢 **SUPPORTED** |
