...
|---|---|---|---|
| **INV-01** | Single Client Identity | $\operatorname{FedAvg}(\{W\}) \equiv W$ for arbitrary weight vectors $W \in \mathbb{R}^d$. | 🟢 **PASS** |
| **INV-02** | Convex Hull Boundedness | $\min_i W_i \le \operatorname{FedAvg}_{\text{weighted}}(\{W_i\}) \le \max_i W_i$ under non-IID sample ratios ($1:500$). | 🟢 **PASS** |
...
| **INV-08** | Zero-Sum Mask Cancellation | $\operatorname{FedAvg}(\{W_i + m_i\}) \equiv \operatorname{FedAvg}(\{W_i\})$ for zero-sum pairwise masks $\sum p_i m_i = \mathbf{0}$. | 🟢 **PASS** |
