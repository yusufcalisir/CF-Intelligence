...
def test_cross_tenant_resource_access_attempt_strictly_blocked() -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        enforce_tenant_isolation(caller_tenant="bank_a", resource_bank_id="bank_b")

    assert exc_info.value.status_code == 403
    assert "Cross-tenant access forbidden" in exc_info.value.detail or "bank_b" in exc_info.value.detail

    # Cross-tenant penetration attempt with case variations
    with pytest.raises(HTTPException) as exc_info2:
        enforce_tenant_isolation(caller_tenant="bank_alpha", resource_bank_id="bank_beta")
    assert exc_info2.value.status_code == 403

    # Same bank access succeeds without exception
    enforce_tenant_isolation(caller_tenant="bank_a", resource_bank_id="bank_a")
    enforce_tenant_isolation(caller_tenant="BANK_A", resource_bank_id="bank_a")
