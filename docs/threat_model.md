...
| **Information Disclosure** | T5 | A4 | **Server Stack Trace & PII Leakage:** Triggering 500 exceptions or validation errors to inspect internal directory trees, database schemas, or raw financial PII in log records. | **Production Error Sanitizer & Masking (`error_handler.py`, `data_validator.py`):** Intercepts all unhandled exceptions; suppresses stack traces, file paths, and SQL queries, returning RFC 7807 problem details with `incident_id`; masks raw IBAN, SSN/TCKN, PAN, Email, and Phone with type-salted HMAC-SHA256 tokens in server logs and dev details; enforces streaming zero-PII gating with bounded quarantine storage. | `tests/unit/test_error_sanitization.py`<br/>*(7/7 error sanitization tests passed)* |
