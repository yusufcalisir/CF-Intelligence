...
* **34 Modular REST Routers** in [`routers/`](../backend/app/presentation/routers): Verifies request formats via Pydantic schemas, enforcing `@limiter.limit(...)`, `enforce_tenant_isolation(...)`, and on-demand adapter probes (`onboarding.py`, `cases.py`, `alerts.py`, `banks.py`, `gateway.py`, `predict.py`, `diagnostics.py`, etc.).
...
- **Non-IID Dirichlet Partitioner ($\text{Dir}(\alpha)$):** Simulates cross-bank label skew ($\alpha \in [0.01, 10.0]$) using class-wise Dirichlet sampling $\mathbf{p}_c \sim \mathrm{Dir}(\alpha \cdot \mathbf{1}_K)$. Rejection sampling (up to 100 attempts) paired with boundary donor rebalancing strictly guarantees $|D_i| \ge \text{min\_size}$ even under extreme skew ($\alpha = 0.01$) without artificial proportion distortion.
...
│   └── tests/                    # 1,566 automated unit, integration, & security tests
├── frontend/
│   ├── src/
...
```
 GitHub Actions Event (Push / PR / Nightly Cron)
     ...
     └── 6. pytest-security-and-compliance-suites (1,621 Automated Pytest Suites)
```

### Comprehensive Test Suite Verification
The entire codebase is validated by **1,621 automated tests** across unit, integration, and property-based suites:

```bash
pytest backend/tests/ -q
# Result: 1,621 tests collected and passing across all domain, application, and infrastructure modules
```
...
| **Full Automated Test Suite**| `Pytest` | 1,621 automated tests covering EU AI Act, Differential Privacy, Spectral Defense, Onboarding, Open Banking PSD2, and DR Failover |
