...
- **ECDSA Digital Signatures**: Each gradient tensor is compressed via `zlib` and digitally signed by the node's HSM / PKI private key:
  ...
  $$\operatorname{Signature} = \operatorname{Sign}_{K_{\mathrm{private}}}\Big(\text{round-id} \mathbin{\Vert} \text{bank-id} \mathbin{\Vert} \operatorname{SHA-256}(\text{compressed-gradient})\Big)$$
