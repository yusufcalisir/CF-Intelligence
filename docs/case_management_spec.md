...
- Compiles XML adhering strictly to `schemas/FinCEN_SAR_2.0.xsd`:
  ...
  - `<Subjects>`: Type-salted HMAC-SHA256 privacy hashes for all involved suspect entities (`EntityPrivacyHash`). Unlinked alerts dynamically derive a deterministic zero-PII subject privacy hash ($(\mathrm{SHA\text{-}256}(\mathrm{prefix}_{\mathrm{subject}} \mathbin{\Vert} \mathrm{id}_{\mathrm{case}}))[0:32]$) eliminating static mock constants.
...
| **EU AMLA & AMLD6** | **Inter-Bank FININT Case Messaging** | Curve25519 ECDH + AES-256-GCM encrypted case exchange between consortium institutions, SHA-256 evidence integrity hashing, SLA countdown timers, and immutable audit trails (detailed in [`docs/european_finint_and_regtech_spec.md`](european_finint_and_regtech_spec.md)). |
| **EPC SCT Inst** | **Payment Recall Automation** | Real-time SEPA Instant payment recall orchestration (`camt.056`/`camt.029`) with 10-day regulatory boundary and automated account freeze (detailed in [`docs/european_finint_and_regtech_spec.md`](european_finint_and_regtech_spec.md)). |
| **UNODC goAML 4.0** | **Electronic FIU Reporting** | Standardized XML schema 4.0 generation for national Financial Intelligence Units (FIUs) with Four-Eyes dual supervisor sign-off (detailed in [`docs/european_finint_and_regtech_spec.md`](european_finint_and_regtech_spec.md)). |
