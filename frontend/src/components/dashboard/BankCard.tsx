...
export default function BankCard({ bank, index }: BankCardProps) {
  ...
  return (
    <motion.div
      ...
      className="glass-card p-4 hover:border-[var(--color-accent-indigo)]/50 transition-all duration-300 flex flex-col justify-between"
    >
      {/* Header */}
      ...
      <div className="mt-2 flex flex-wrap gap-1">
        {bank.characteristics.slice(0, 2).map((c) => (
          <span
            key={c}
            className="text-[9px] px-1.5 py-0.5 rounded-full bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)] border border-[var(--color-border)]"
          >
            {c}
          </span>
        ))}
