...
telemetry.on('round.stage', (evt) => {
  console.log(\`Round \${evt.roundId}: Accuracy \${evt.accuracy}%\`);
        ...
        {/* ── FOOTER ──────────────────────────────────────────────── */}
        <footer className="border-t border-white/6 py-10 sm:py-12 px-4 sm:px-6 w-full min-w-0 bg-[#05050f]/60">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left min-w-0">
            <div className="flex flex-col sm:flex-row items-center gap-3 min-w-0">
              <BrandLogo className="w-8 h-8 sm:w-8 sm:h-8 shrink-0" />
            ...
            <div className="text-[10.5px] sm:text-xs font-mono text-slate-400 break-words text-center">
              PyTorch · Intel SGX · ISO 20022 · FinCEN SAR
            </div>

            <button
              onClick={() => navigate('/dashboard')}
              className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 transition-all cursor-pointer shadow-[0_0_20px_rgba(99,102,241,0.4)] justify-center shrink-0"
            >
