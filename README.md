...
| :--- | :--- | :--- |
| **Real-Time Load & Latency SLA** | `python scripts/run_load_test.py --concurrency 3 --requests 1000 --pacing-ms 10.0` or `locust -f scripts/locustfile.py --headless -u 50 -r 10 -t 60s` | Empirical high-concurrency load test measuring p50, p95, and p99 inference latency distributions across multi-bank payment streams (sub-15ms raw scoring, ~100–180ms p50/p95 under full feature store enrichment; tail p99 bounded under concurrency). Generates `reports/load_test_report.md` and `storage/load_test_results.json`. |
